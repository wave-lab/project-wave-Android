...
class SearchFragment : Fragment(), OnBackPressedListener{
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        v.ll_search_frag_focus_on.visibility = View.GONE
        songDataArr = ArrayList<SongData>()
        coverArtistDataArr = ArrayList<CoverArtistData>()
        requestManager = Glide.with(this)

        // Edittext focus off
        v.ll_search_frag_focus.setOnClickListener {
            searchEditTextFocusOff()
        }
        ...
        v.recycler_search_frag_song.isNestedScrollingEnabled = false
        ...
        v.recycler_search_frag_artist .isNestedScrollingEnabled = false

        insertSearchHistoryData(v)

        // Edittext focus ON
        v.edit_search_frag_searchbar.setOnFocusChangeListener { view, hasFocus ->
            if(hasFocus){
                searchEditTextFocusOn()
            }
        }

        v.tv_search_frag_all_delete.setOnClickListener {
            searchData.clear()
            searchDataHistoryAdapter = SearchDataHistoryAdapter(searchData)
            v.recycler_search_frag_search_hisory.adapter = searchDataHistoryAdapter
            v.recycler_search_frag_search_hisory.layoutManager = LinearLayoutManager(context)
            v.recycler_search_frag_search_hisory.isNestedScrollingEnabled = false
        }
    ...
    fun searchEditTextFocusOff(){
        ll_search_frag_focus_off.visibility = View.VISIBLE
        ll_search_frag_focus_on.visibility = View.GONE
        edit_search_frag_searchbar.clearFocus()
        searchBackFlag = 0
    }

    fun searchEditTextFocusOn(){
        ll_search_frag_focus_off.visibility = View.GONE
        ll_search_frag_focus_on.visibility = View.VISIBLE
        searchBackFlag = 1
    }
